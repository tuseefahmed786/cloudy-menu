...
function PerProduct() {
    ...
    if (isLoading) return <Isloading width="w-14" height="h-14" optionaltext={`Loading ${productName}`} />;
            ...
            <div className="h-screen overflow-hidden max-w-[25rem] mx-auto bg-white h-full shadow-xl top-0 left-0 right-0">
                ...
                <div className='flex w-full flex-col border-b border-b-[#80808057]'>
                    <div className='w-full h-64'>
                        <img className='object-cover w-full h-full' src={restaurantData.imageUrl} alt='pic here product' />
                    </div>
                    <div className='w-full px-3 pt-4 flex-col flex gap-1'>
                        <div className='flex justify-between items-center'>
                            <h1>{restaurantData.name}</h1>
                            <h1 className='text-base'>AED {restaurantData.price}</h1>
                        </div>

                        <p className='text-sm py-2 text-gray-600'>{restaurantData.description}</p>
                    </div>
